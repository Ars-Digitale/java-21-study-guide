...
<br>Finally, an expression or a code block with braces `({})`, for the code to execute, when a match occurs; <br>(Note that if you use the arrow operator for one clause, you must use it for all clauses)
